...
* **Estrutura (HTML5 Semântico):** A página utiliza tags como `<header>`, `<nav>`, `<main>`, `<section>` e `<footer>` para garantir uma estrutura clara e acessível.
* **Estilo e Layout (CSS3):**
    * Utilizei o **CSS Grid Layout** para organizar a vitrine de produtos e garantir que o layout se adaptasse perfeitamente a telas de desktops, tablets e celulares (Design Responsivo).
    * Apliquei efeitos simples como `transition` e `box-shadow` nos botões e cards de produto.
* **Interatividade (JavaScript):**
    * **Carrinho de Compras:** A principal funcionalidade é o carrinho. A lógica inclui:
        1.  Uso do `addEventListener` para capturar o clique do botão "Adicionar".
        2.  **Cálculo** do valor total dos produtos (`let total = 0;`).
...
* **JavaScript (Puro)**
* **Git & GitHub** (Utilizado para versionamento e entrega do link público)
